...
class PathSetI(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def handle(self, feature_a_list, feature_b_list, compare_func) -> (int, int): ...  # 处理数据，得到最相似的两帧索引
...
class PathSetImpl(PathSetI):
    ...
    def handle(self, feature_a_list, feature_b_list, compare_func) -> (int, int):
        ...
        return min_a_index, min_b_index  # 返回差异值最小的 a_index 和 b_index
    ...
    def run(self):
        ...
        while True:
            ...
            if len(feature_queue_a) >= self.pool_size:
                ...
                self.pbar.update(self.pool_size - 50)
