...
class SurfModelSingleImpl:
    _instance = None

    def __new__(cls):
        if cls._instance is None:
            cls._instance = super().__new__(cls)
            cls._instance.model = cv2.xfeatures2d.SURF_create()
            print("inited model")
...
class TwoPFastDtwSurfImpl(TwoPFastDtwSiftImpl):
    @staticmethod
    def get_distance(feature_a, feature_b) -> float:
        ...
        keypoints1, descriptors1 = SurfModelSingleImpl().get_model().detectAndCompute(feature_a, None)
        keypoints2, descriptors2 = SurfModelSingleImpl().get_model().detectAndCompute(feature_b, None)
